...
/**
 * How long `slow` mode stalls *after* writing, so the client times out without
 * learning the outcome. The default comfortably outlasts the client's 10s
 * timeout in the app while staying short enough for the test suite, which uses
 * a 2s timeout. Override with SLOW_DELAY_MS for a longer manual demo.
 */
const SLOW_DELAY_MS = Number(process.env.SLOW_DELAY_MS || 12_000);
